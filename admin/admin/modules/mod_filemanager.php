...
if ($ck == 1) { /* Here the dialogu sends ck=1 in the request url and only if that happens do we do the following : */
?>
<style type="text/css">
#nav_level_1, #pixie_title span, #pixie_footer, #admin_block_tags, #tools {
display:none; }
html {
					...
					<div class=\"admin_block\" id=\"admin_block_filemanager\">
						...
						echo "</div>
							...
							<div class=\"form_row_button\">
								...
								if ($ck) { echo "<input type=\"hidden\" name=\"ckFuncNumReturn\" value=\"$CKEditorFuncNum\" />"; }
								if ($ck) { echo "<input type=\"hidden\" name=\"ck\" value=\"1\" />"; }
								if ($ckfile) { echo "<input type=\"hidden\" name=\"ckfile\" value=\"1\" />"; }
								if ($ckimage) { echo "<input type=\"hidden\" name=\"ckimage\" value=\"1\" />"; }
	echo "				</div>
						</fieldset>
						</form>
					</div>";
	...
	echo "
				<div id=\"pixie_content\" class=\"filemanager\">
					<h2>" . $lang['nav2_files'] . "</h2>";
						if ($ck) { $messageok = 'Click on a file by name to create a link.'; } /* Needs language */
	echo "					<div id=\"filemanager_table\">
						...
						<thead>
							<tr>
								...
								if (!$ck) { echo "<th class=\"tbl_heading\" id=\"thedatestamp\">" . $lang['filedate'] . "</th>"; }
			...
			if ($file_type == 'Image') {
				...
				echo "
						<tr class=\"$trclass\">
							...
							if (!$ck) { echo "<td class=\"tbl_row\" headers=\"thefilename\"><a href=\"../files/images/$file_name\" class=\"thickbox\" alt=\"/images/$file_name\">" . $file_name . "</a></td>"; }
							if ($ck) { echo "<td class=\"tbl_row\" headers=\"thefilename\"><a href=\"../files/images/$file_name\" class=\"pxfinder\" alt=\"/images/$file_name\">" . $file_name . "</a></td>"; }
							if (!$ck) { echo "<td class=\"tbl_row\" headers=\"thedatestamp\">" . safe_strftime($date_format, $last_modified) . "</td>"; }
				echo "			<td class=\"tbl_row\" headers=\"thetags\">$tags</td>
