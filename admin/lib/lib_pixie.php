...
// Build the navigation dynamically or build it from specified array
	function build_navigation()
	{
		global $s, $site_url, $nested_nav, $lang;
   			...
   			$check_pages = safe_rows("*","pixie_core", "public = 'yes' and in_navigation = 'yes' order by page_order asc");
			...
			while ($i < $num){
  			...
  			if (($page_name == "404") || ($page_name == "rss")) {
  			//	do nothing
  			} else {
  				if ($s == $page_name) {
					...
					if (($nested_nav == "yes") && (file_exists('admin/blocks/block_'.$includestr.'_nav.php'))) {
	  					print "\t\t\t\t\t<li id=\"li_1_$page_name\"><a href=\"".createURL($page_name)."\" title=\"$page_display_name\" id=\"navigation_1_$page_name\" class=\"nav_current_1 replace\">$page_display_name<span></span></a>\n";
						include('admin/blocks/block_'.$includestr.'_nav.php');
	  				} else {
	  					print "\t\t\t\t\t<li id=\"li_1_$page_name\" class=\"nav_current_li_1\"><a href=\"".createURL($page_name)."\" title=\"$page_display_name\" id=\"navigation_1_$page_name\" class=\"nav_current_1 replace\">$page_display_name<span></span></a></li>\n";
	  				}
  				} else {
  					print "\t\t\t\t\t<li id=\"li_1_$page_name\"><a href=\"".createURL($page_name)."\" title=\"$page_display_name\" id=\"navigation_1_$page_name\" class=\"replace\">$page_display_name<span></span></a></li>\n";  			
  				}
  			}
			$i++;
			}
	...
	function build_title($ptitle='') {
		...
		if ($ptitle) {
			...
			if ($page_type == "dynamic" && $m == "permalink") {
				...
				print "$site_name - $page_display_name - $post_title";
			} else if ($m == "tag") {
				if ($p) {
					print "$site_name - $page_display_name - Tag - ".simplify(squash_slug($x))." - Page $p";
				} else {
					print "$site_name - $page_display_name - Tag - ".simplify(squash_slug($x));
				}
			} else if ($m == "page") {
				print "$site_name - $page_display_name - ".simplify($m)." ".simplify($x);		
			} else if ($m) {
				print "$site_name - $page_display_name - ".simplify($m);		
			} else {
				print "$site_name - $page_display_name";
			}
		}
	}
?>